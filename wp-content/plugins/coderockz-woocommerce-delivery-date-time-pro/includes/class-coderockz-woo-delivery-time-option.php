	...
	class Coderockz_Woo_Delivery_Time_Option {
		...
		public static function delivery_time_option($delivery_time_settings,$meta_box=null) {
			...
			if($enable_as_soon_as_possible_option) {
				$result["as-soon-as-possible"] = $as_soon_as_possible_text;
			}
			...
			if($enable_custom_time_slot) {
				if(isset($custom_time_slot_settings['time_slot']) && count($custom_time_slot_settings['time_slot'])>0){
					...
					if($enable_as_soon_as_possible_option) {
						$custom_result["as-soon-as-possible"] = $as_soon_as_possible_text;
					}
			  		...
			  		foreach($sorted_custom_slot as $individual_time_slot) {
			  			...
			  			if($individual_time_slot['enable'] ) {
			  				...
			  				$timeslot_hide_categories = (isset($individual_time_slot['hide_categories']) && !empty($individual_time_slot['hide_categories'])) ? $individual_time_slot['hide_categories'] : array();
							...
							if($hide_categories_condition) {
								...
								if($individual_time_slot['time_slot_shown_other_categories_products'] && count($checkout_product_categories) > 1) {
					  				$hide_for_product_categories = !$hide_for_product_categories;
					  			}
							} elseif($hide_products_condition) {
								$hide_for_product_categories = true;
								if($individual_time_slot['time_slot_shown_other_categories_products'] && count($checkout_products) > 1) {
					  				$hide_for_product_categories = !$hide_for_product_categories;
					  			}
							} else {
			  				...
			  				if(class_exists('WOOCS_STARTER')){
								...
								$fee = (isset($individual_time_slot['fee']) && $individual_time_slot['fee'] != "") ? " (+".$currency_symbol.apply_filters('woocs_exchange_value', $individual_time_slot['fee']).")" :"";
							} else {
								$fee = (isset($individual_time_slot['fee']) && $individual_time_slot['fee'] != "") ? " (+".$currency_symbol.$individual_time_slot['fee'].")" :"";
							}
			...
			if($enable_time_slot_fee) {
				...
				foreach($result as $key => $value) {
					$slot_key = "";
					if($key != "as-soon-as-possible" && $key != ""){
					...
					if(isset($slot_fees[$slot_key])) {
						if($slot_fees[$slot_key] > 0) {
							if(class_exists('WOOCS_STARTER')){
                            	...
                            	$currencies=$WOOCS->get_currencies();
                            	$currency_symbol = $currencies[$WOOCS->current_currency]['symbol'];
								$fee = " (+".$currency_symbol.apply_filters('woocs_exchange_value', $slot_fees[$slot_key]).")";
							} else {
								$fee = " (+". $currency_symbol . $slot_fees[$slot_key] . ")";
							}
